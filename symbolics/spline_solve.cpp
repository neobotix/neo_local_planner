...
int main()
{
	...
	std::cout << "spline_0_dt: " << spline_0_dt << std::endl;
	std::cout << "spline_1_dt: " << spline_1_dt << std::endl;

	lst spline_0_ddt = ex_to<lst>(diff(spline_0_dt, t));
	lst spline_1_ddt = ex_to<lst>(diff(spline_1_dt, t));

	std::cout << "spline_0_ddt: " << spline_0_ddt << std::endl;
	std::cout << "spline_1_ddt: " << spline_1_ddt << std::endl;

	lst spline_0_dddt = ex_to<lst>(diff(spline_0_ddt, t));
	lst spline_1_dddt = ex_to<lst>(diff(spline_1_ddt, t));

	std::cout << "spline_0_dddt: " << spline_0_dddt << std::endl;
	std::cout << "spline_1_dddt: " << spline_1_dddt << std::endl;

	lst equations = {
		spline_0[0].subs(t == 0) == p_x,			// start position
		spline_0[1].subs(t == 0) == p_y,			// start position
		spline_1[0].subs(t == 1) == q_x,			// end position
		spline_1[1].subs(t == 1) == q_y,			// end position
		spline_0_dt[0].subs(t == 0) == dp_x,		// start orientation + velocity
		spline_0_dt[1].subs(t == 0) == dp_y,		// start orientation + velocity
		spline_1_dt[0].subs(t == 1) == dq_x,		// end orientation + velocity
		spline_1_dt[1].subs(t == 1) == dq_y,		// end orientation + velocity
		spline_0_ddt[0].subs(t == 0) == ddp_x,		// start yawrate + acceleration
		spline_0_ddt[1].subs(t == 0) == ddp_y,		// start yawrate + acceleration
		// spline_1_ddt[0].subs(t == 1) == 0,		// end yawrate + acceleration
		// spline_1_ddt[1].subs(t == 1) == 0,		// end yawrate + acceleration
		...
		spline_0[0].subs(t == 1) == spline_1[0].subs(t == 0),			// position connectivity
		spline_0[1].subs(t == 1) == spline_1[1].subs(t == 0),			// position connectivity
		spline_0_dt[0].subs(t == 1) == spline_1_dt[0].subs(t == 0),		// orientation + velocity connectivity
		spline_0_dt[1].subs(t == 1) == spline_1_dt[1].subs(t == 0),		// orientation + velocity connectivity
		spline_0_ddt[0].subs(t == 1) == spline_1_ddt[0].subs(t == 0),	// yawrate + acceleration connectivity
		spline_0_ddt[1].subs(t == 1) == spline_1_ddt[1].subs(t == 0)	// yawrate + acceleration connectivity
	};

	lst unknowns = {
		coeff_0[0][0], coeff_0[0][1], coeff_0[0][2], coeff_0[0][3],
		coeff_0[1][0], coeff_0[1][1], coeff_0[1][2], coeff_0[1][3],
		coeff_1[0][0], coeff_1[0][1], coeff_1[0][2], coeff_1[0][3],
		coeff_1[1][0], coeff_1[1][1], coeff_1[1][2], coeff_1[1][3]
	};
	...
	ex result_2 = ex_to<lst>(ex(spline_1).subs(solution));
	ex result_2_dt = ex_to<lst>(ex(spline_1_dt).subs(solution));
